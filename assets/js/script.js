...
$(document).ready(function () {
	...
	$('#submit-contact-form').on('click', function (e) {
		...
		$.post(
			...
			function (data) {
				if (data == 'success') {
					$('.form-area').hide()
					$('.status').html('Form Successfully Submitted.').css('margin-top', '100px')
				}
				if (data == 'failed') {
					$('.status').html('Failed To submit the form! Please try again.').css('margin-top', '100px')
					$('#submit-contact-form').val('Send Again')
	...
	$('.reg_submit').click(function (e) {
		$.post(
			...
			function (data) {
				if (data == 'success') {
					$('.register-section .container .row').hide()
					$('.data-submitted').html('Thank you to subscribe our newsletter.')
				}
				if (data == 'failed') {
					$('#failed').html('Please try again with another mail.')
				}
			}
		)
		return false
...
window.pressed = function () {
	...
	if (a.value == "") {
		fileLabel.innerHTML = "Upload Your File";
	} else {
