...
$(document).ready(function () {
	...
	$('#submit-contact-form').on('click', function (e) {
		...
		$.post(
			...
			function (data) {
				if (data == 'success') {
					$('.form-area').hide()
					$('.status').html('Form Successfully Submitted.')
				}
				if (data == 'failed') {
					$('.status').html('Failed To submit the form! Please try again.')
	...
	$('#team-submit-contact-form').on('click', function (e) {
		$('#team-submit-contact-form').val('sending...')
		$.post(
			'message.php',
			{
				name: $('#name').val(),
	...
	var target = $('.inline-block a[href="' + path + '"]')
	target.addClass('active-menu')

	var nestedPath = window.location.pathname.split('/')
	if (nestedPath[3] == 'blog') {
		var target = $('.inline-block a[href="blog.php"]')
		target.addClass('active-menu')
	}
	if (nestedPath[3] == 'game') {
		var target = $('.inline-block a[href="games.php"]')
		target.addClass('active-menu')
	}
	if (nestedPath[3] == 'team') {
		...
		gamesChar.on('inview', function (event, isInView) {
			if (isInView) {
				if (!$(this).hasClass('visible')) {
					...
					progressBarCreator(progressline90, 0.9)
					progressBarCreator(progressline50, 0.5)
					progressBarCreator(progressline85, 0.85)
					progressBarCreator(progressline55, 0.55)
				}
			}
		})
	}
